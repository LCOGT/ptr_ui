    ...
    <div class="container phase-status-wrapper">
        ...
        <template v-for="(s, index) in phase_status_sorted">
            ...
            <b-tooltip
                ...
                type="is-dark"
            >
                <span class="phase-status-text" :style="text_opacity_style(s.timestamp)" >
                    {{s.message}}
                </span>
            </b-tooltip>
            <span v-else :key="index" class="phase-status-text" :style="text_opacity_style(s.timestamp)">
                {{s.message}}
            </span>
...
export default {
    ...
    methods: {
        ...
        text_opacity_style(timestamp) {
            const hour = 60 * 60
            let opacity_percent = 100 * (1 - (Math.min((this.now - timestamp) / hour, 0.5)))
    ...
    computed: {
        ...
        phase_status_sorted() {
            let sorted = this.phase_status.sort((a,b) =>  b.timestamp - a.timestamp )
            return sorted
        }
    }
