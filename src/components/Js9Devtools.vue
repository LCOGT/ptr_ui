    ...
    >
      ...
      <div class="js9-dev-buttons">
        <div class="server-tasks">
          <div
            :disabled="!js9IsVisible"
            class="button"
            @click="JS9.RunAnalysis('macros')"
          >
            Show Macros
          </div>
          <div
          ...
          >
            <b-field label="crosshairs">
              <b-switch
                v-model="crosshairIsActive"
                type="is-light"
                @input="toggleCrosshair"
              />
            </b-field>
          </b-tooltip>
          <hr>
...
export default {
  ...
  data () {
    return {
      JS9, // grab the global variable for use in our component
      plot1: ''
    }
  },
  methods: {
    ...
    displayMyAnalysis: function (stdout, stderr, errcode, a) {
      const rdiv = document.getElementById('js9-plot-1')
      if (stderr) {
        alert(stderr)
      } else {
