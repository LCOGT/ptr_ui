...
//!(function () {
  ...
  Celestial.display = function (config) {
    ...
    d3.select("#celestial-map canvas").on('contextmenu', function() {
      ...
      //    if (xydistance(mousexy, objxy) < radius_nearby) {
      //      var object_with_xy = td[i];
      //      object_with_xy.push(objxy); // Append xy coordinates so it doesn't have to be recalculated
      //      close_objects.push(object_with_xy);
      //    };
      //  };
      //};
      //if (close_objects) {
...
*/
    ...
    if (cfg.controls === true && $("celestial-zoomin") === null) {
      d3.select(par).append("input").attr("type", "button").attr("id", "celestial-zoomin").attr("value", "\u002b")
        .on("click", function () { zoomBy(1.25); return false; });
      d3.select(par).append("input").attr("type", "button").attr("id", "celestial-zoomout").attr("value", "\u2212")
        .on("click", function () { zoomBy(0.8); return false; });
    }
  ...
  */
  ...
  var Kepler = function () {
    ...
    kepler.thisname = function (_) {
      if (!arguments.length) return thisname;
      thisname = _;
      return kepler;
    };
