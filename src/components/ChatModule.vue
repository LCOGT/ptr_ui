<template>
  <div class="wrapper">
    <b-collapse
      class="card"
      :open.sync="chatIsOpen"
      @open="readAllMessages"
    >
      <div
        slot="trigger"
        slot-scope="props"
...
export default {
  ...
  methods: {
    ...
    getRecentMessages () {
      const data = { action: 'getRecentMessages', room: this.sitecode }
    ...
    getOnlineUsers () {
      const onlineUserURL = `https://327l4vns8i.execute-api.us-east-1.amazonaws.com/dev/onlineusers?room=${encodeURIComponent(this.sitecode)}`
    ...
    calculateUnreadMessages () {
      const lastSeenTimestamp = this.getLastSeenTimestamp(this.username)
      let count = 0
    ...
    readAllMessages () {
      const lastReadTime = this.messages.slice(-1)[0].timestamp
