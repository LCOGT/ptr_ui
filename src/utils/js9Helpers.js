...
function quickselect_median (arr) {
  const L = arr.length; const halfL = L / 2
  if (L % 2 == 1)
...
try {
  JS9Helpers.drawCrosshairCuts = function (im, ipos, evt) {
    if (im && im.crosshair && im.params.crosshair && evt.key == 'Shift') {
      ...
      const xOutDiv = document.getElementById('js9-x-profile')
      const yOutDiv = document.getElementById('js9-y-profile')

      // Get data from the displayed image.
      const obj = JS9.GetImageData(true)
      ...
      const iposX = Math.floor(ipos.x - 0.5)
      const iposY = Math.floor(ipos.y - 0.5)
      ...
      for (x = 0; x < obj.dwidth; x += 1) {
        ...
        imX = JS9.DisplayToImagePos({ x, y: 0 }).x
      ...
      for (let y = 0; y < obj.dheight; y += 1) {
        imY = JS9.DisplayToImagePos({ x: 0, y }).y
