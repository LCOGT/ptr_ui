...
function tz_name_from_coordinates(lat, lng) {
    ...
    let tz_name = timespace.getFuzzyLocalTimeFromPoint(new Date(), location)   
    console.log(tz_name)
    try {
        return tz_name.tz()
    } catch(e) {
...
function utc_offset_from_coordinates(lat, lng, date = new Date()) {
    try {
        let tz_name = tz_name_from_coordinates(lat, lng)
        let offset = get_offset(tz_name, date) / 60  // convert to hours
        ...
        if (err instanceof CoordinateTimezoneError) {
            let offset = offset_from_longitude(lng) 
