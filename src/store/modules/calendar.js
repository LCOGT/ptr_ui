...
const getters = {
    ...
    endOfUserReservation: state => user_id => {
        let end_times = state.active_reservations
    ...
    fetchActiveReservations({ commit, dispatch }, site) {
        ...
        axios.post(url, request_body).then(response => {
            ...
            if (response.data.length > 0) {
                ...
                let expirations = response.data.map(reservation => 
                    moment(reservation.end).valueOf())
                let soonest = Math.min(...expirations)

                // additional milliseconds before we rescan for reservations
                let buffer_time = 1500
                let time_to_reservation_end = (soonest + buffer_time) - moment().valueOf()  
